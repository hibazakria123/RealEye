  ...
  return (
    <section id="results" className="bg-ink px-6 pb-32 pt-4 text-cream">
      <div className="mx-auto max-w-7xl">
        ...
        {results ? (
          <FinalVerdict results={results} onReset={onReset} />
        ) : null}
      </div>
    </section>
  );
  ...
  return (
    ...
    >
      <div className="rounded-[22px] bg-ink-soft px-8 py-12 text-center">
          ...
          {[results.modelA, results.modelB, results.modelC].map((v) => (
            ...
            >
              ...
              <span
                ...
                style={{
                  color: v.prediction === "REAL" ? "#5CB85C" : "#E25C5C",
                }}
              >
                {v.prediction}
              </span>
        ...
        {results.regionScores &&
          Object.keys(results.regionScores).length > 0 && (
            <div className="mt-10 text-left">
              <p className="mb-5 text-center text-[11px] font-semibold uppercase tracking-[0.28em] text-mute">
                Face Region Analysis
              </p>

              <div className="space-y-4">
                {Object.entries(results.regionScores)
                  .sort((a, b) => b[1] - a[1])
                  .map(([region, score]) => {
                    const isSuspicious = score >= 50;
                    const barColor = isSuspicious ? "#E25C5C" : "#5CB85C";

                    return (
                      <div key={region} className="flex items-center gap-3">
                        <span className="w-20 shrink-0 text-right text-xs text-mute">
                          {region}
                        </span>

                        <div className="relative h-2 flex-1 overflow-hidden rounded-full bg-cream/10">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${score}%` }}
              ...
              <p className="mt-4 text-center text-[10px] text-mute">
                Red = Suspicious Region &nbsp;·&nbsp; Green = Real Region
              </p>
            </div>
          )}
